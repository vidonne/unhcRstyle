...
#' Default: \code{pal = \link{pal_unhcr}}.
#'
...
#'
#' @examples
#' usecol(pal = pal_unhcr, n = "all")  # default color palette
#' usecol(pal = pal_unhcr, n =  4)     # selecting n dedicated colors
#' usecol(pal = pal_unhcr, n = 20)     # extending color palette
#'
...
usecol <- function(pal = pal_unhcr,
  ...
  if ( !use_col_ramp ) {
    ...
    if ( any(pal_ix) & length(pal_inp) >= n) {
      ...
      set1 <- pal_name %in% c("pal_blue",
                              "pal_grey",
                              "pal_green",
                              "pal_red",
                              "pal_yellow")
      set2 <- pal_name %in%   "pal_darkblue"
      set3 <- pal_name %in%   "pal_unhcr"
      set4 <- pal_name %in% c("pal_unhcr_primary",
                              "pal_unhcr_graphic")
      ...
      out_col <- switch(pal_set,
                        ...
                        switch(
                          ...
                          # 10
                          pal  # all 11 colors of pal_unhcr
                        ),
...
seecol <- function(pal = "unhcr_all",  # which palette to output?
  ...
  if (by_key) {
    ...
    if (is.na(title)){
      if (pal %in% c("all", "unhcr_all", "all_unhcr") ) title <- "See all unhcr color palettes"
